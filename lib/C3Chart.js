...
var C3Chart = React.createClass({
	...
	multiLineDataPreparator: function(rawData) {
		var base;
		base = _.map(rawData, function(datum) {
		  return datum.values.map(function(d) {
				var xLabels = datum.key;
				var obj =  {"label": d.label }
				obj[xLabels] = d.value
				return obj
			});
		});
		var data;
		data = _.merge.apply(_,base)
		return data
	},

	drawGraphMultiLine: function() {
		console.log("drawing multi line");
		var graphObject = this.graphObject();
		var graphObjectData = {
			json: this.multiLineDataPreparator(this.props.data),//.values,
			keys: { x: "label", value: _.map(this.props.data, 'key') },
                        names: { value: _.map(this.props.data, 'key') }
		};
		var graphObjectAxis = {
			x: { type: "category" } // this needed to load string x value
		};
